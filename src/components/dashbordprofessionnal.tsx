...
const DashbordProfessionnal: React.FC = () => {
  ...
  const [editingProfile, setEditingProfile] = useState<Professional | null>(
    null
  );
  ...
  const handlePayment = async (method: string) => {
    ...
    try {
      ...
      if (success) {
        ...
        alert(
          `Paiement réussi via ${method}! Votre abonnement Pro est maintenant actif.`
        );
        ...
        <div className="flex-1 p-4 md:p-8">
              ...
              <div className="px-4 py-5 sm:px-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                  <div>
                    ...
                    {profile?.specialite || user?.specialite || "N/A"}
                  </div>
                  <div>
                    <strong>Diplôme:</strong>{" "}
                    {profile?.diplome || user?.diplome || "N/A"}
                  </div>
                  <div>
                    <strong>Expérience:</strong>{" "}
                    {profile?.experience || user?.experience || "N/A"}
                  </div>
                  <div>
                    <strong>Adresse:</strong>{" "}
                    {profile?.adresse || user?.adresse || "N/A"}
                  </div>
                  <div>
                    <strong>Téléphone:</strong>{" "}
                    {profile?.telephone || user?.telephone || "N/A"}
                  </div>
                  <div>
                    <strong>Tarif:</strong>{" "}
                    {profile?.tarif || user?.tarif || "N/A"}
                  </div>
                  <div className="md:col-span-2">
                    <strong>Bio:</strong> {profile?.bio || user?.bio || "N/A"}
                  </div>
                  <div className="md:col-span-2">
                    <strong>Langues:</strong>{" "}
                    {(profile?.langues || user?.langues || []).join(", ") ||
                      "N/A"}
                  </div>
                  <div className="md:col-span-2">
                    ...
                    <ul className="list-disc list-inside mt-1">
                      {(
                        profile?.creneauxDisponibles ||
                        user?.creneauxDisponibles ||
                        []
                      ).map((creneau, index) => (
                        <li key={index}>
                          {creneau.jour}: {creneau.heures.join(", ")}
                        </li>
                      ))}
                    </ul>
                  </div>
                    ...
                    <div className="flex items-center">
                      ...
                      <div className="ml-4">
                        ...
                        <p className="text-2xl font-bold text-green-800">
                          {
                            rendezVous.filter((r) => r.statut === "confirmed")
                              .length
                          }
                        </p>
                      </div>
                    ...
                    <div className="flex items-center">
                      ...
                      <div className="ml-4">
                        ...
                        <p className="text-2xl font-bold text-yellow-800">
                          {
                            rendezVous.filter((r) => r.statut === "pending")
                              .length
                          }
                        </p>
                      </div>
                    ...
                    <div className="flex items-center">
                      ...
                      <div className="ml-4">
                        ...
                        <p className="text-2xl font-bold text-red-800">
                          {
                            rendezVous.filter((r) => r.statut === "cancelled")
                              .length
                          }
                        </p>
                      </div>
          ...
          {activeTab === "appointments" && (
            <div className="bg-white shadow overflow-hidden sm:rounded-md">
              ...
              {loading && (
                <p className="px-4 py-4">Chargement des rendez-vous…</p>
              )}
              {error && (
                <div className="px-4 py-4 text-red-600">Erreur: {error}</div>
              )}
                      ...
                      <tr key={r.id}>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                          <div className="flex space-x-2">
                            ...
                            <button
                              onClick={() =>
                                updateRendezVousStatus(r.id, "confirmed")
                              }
                            ...
                            <button
                              onClick={() =>
                                updateRendezVousStatus(r.id, "cancelled")
                              }
                ...
                {user?.subscription === "pro" ? (
                  <div className="text-center">
                    ...
                    <p className="text-gray-600 mb-4">
                      Votre abonnement Pro est actif. Vous bénéficiez d'une
                  ...
                  <div className="text-center">
                    ...
                    <p className="text-gray-600 mb-6">
                      Avec l'abonnement Pro, votre profil sera mis en avant et
                      vous recevrez plus de demandes de rendez-vous.
                    </p>
                    <button
        ...
        {selectedRdv && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center">
            <div className="bg-white p-6 rounded shadow max-w-md w-full">
              ...
              <p>
                <strong>Date:</strong>{" "}
                {new Date(selectedRdv.date).toLocaleString()}
              </p>
              <p>
              ...
              <p>
                <strong>Utilisateur ID:</strong> {selectedRdv.utilisateurId}
              </p>
        ...
        {showSubscriptionModal && (
          <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50 flex items-center justify-center">
            <div className="bg-white p-6 rounded shadow max-w-md w-full mx-4">
              ...
              <p className="text-gray-600 mb-6">
                Choisissez votre méthode de paiement pour l'abonnement Pro à 15
                000 CFA / mois.
              </p>
