...
export const getAvisByProfessionnel = async (professionnelId: string): Promise<Avis[]> => {
  try {
    const response = await fetch(`${API_BASE_URL}/avis?professionnelId=${professionnelId}&statut=approuve`);
...
export const getAvisStats = async (professionnelId: string) => {
  try {
    const response = await fetch(`${API_BASE_URL}/avis/stats?professionnelId=${professionnelId}`);
    
    if (!response.ok) {
      throw new Error('Erreur lors de la récupération des statistiques des avis');
    }
...
export const canUserReview = async (professionnelId: string, utilisateurId: string): Promise<boolean> => {
  try {
